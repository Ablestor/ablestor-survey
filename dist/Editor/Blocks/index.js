...
export var BlockPresenter = function (_a) {
    ...
    return (_jsxs(BlockContainer, __assign({ className: classnames({
            required: block.required,
                ...
                block.type === BlockTypes.DROPDOWN) && (_jsx(Row, { children: _jsx(OptionEditor, { options: block.format.options, onChange: function (options) { return onUpdateBlock(__assign(__assign({}, block), { format: __assign(__assign({}, block.format), { options: options }) })); } }, void 0) }, void 0)), block.type === BlockTypes.RANGE && (_jsxs(_Fragment, { children: [_jsx(Row, { children: _jsxs(FlexContainer, { children: [_jsx(FlexElement, __assign({ width: 60 }, { children: _jsx(Text, { children: "\uCD5C\uC18C \uAC12" }, void 0) }), void 0), _jsx(FlexElement, __assign({ width: 'flex' }, { children: _jsx(Select, { items: minRange, selectedIndex: minRange.findIndex(function (r) { return r.value === Number(block.format.options[0]); }), onChange: function (value) {
                                            var options = Array.from({ length: Number(block.format.options[block.format.options.length - 1]) }, function (_, i) { return String(i + value); });
                                            onUpdateBlock(__assign(__assign({}, block), { format: __assign(__assign({}, block.format), { options: options }) }));
                                        } }, void 0) }), void 0), _jsx(FlexElement, __assign({ width: 60 }, { children: _jsx(Text, { children: "\uCD5C\uB300 \uAC12" }, void 0) }), void 0), _jsx(FlexElement, __assign({ width: 'flex' }, { children: _jsx(Select, { items: maxRange, selectedIndex: maxRange.findIndex(function (r) { return r.value === Number(block.format.options[block.format.options.length - 1]); }), onChange: function (value) {
                                            var options = Array.from({ length: value }, function (_, i) {
                                                return String(i + Number(block.format.options[0]));
                                            });
                                            onUpdateBlock(__assign(__assign({}, block), { format: __assign(__assign({}, block.format), { options: options }) }));
